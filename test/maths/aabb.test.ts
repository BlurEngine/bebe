...
import { AABB, isAABBLike, toAABB } from "@blurengine/bebe/maths";
...
describe("maths/AABB", () => {
    it("normalizes corners and exposes derived geometry", () => {
        ...
        expect(box.extent.toObject()).toEqual({ x: 2, y: 1, z: 1.5 });
    ...
    it("builds from helpers and supports spatial queries", () => {
        ...
        const fromCenterExtent = AABB.fromCenterExtent(
            { x: 4, y: 5, z: 6 },
            { x: -1, y: 2, z: -3 },
        );
        ...
        expect(fromCenterExtent.min.toObject()).toEqual({ x: 3, y: 3, z: 3 });
        expect(fromCenterExtent.max.toObject()).toEqual({ x: 5, y: 7, z: 9 });
        expect(fromCenterExtent.extent.toObject()).toEqual({
            x: 1,
            y: 2,
            z: 3,
        });
        ...
        expect(
            fromPoints
                .intersection(new AABB(0, 0, 0, 2, 2, 2))
                ?.toBlockBoundingBox(),
        ).toEqual({
            min: { x: 0, y: 0, z: 1 },
            max: { x: 2, y: 2, z: 2 },
        ...
        expect(
            fromPoints
                .union(new AABB(10, 10, 10, 12, 12, 12))
                .toBlockBoundingBox(),
        ).toEqual({
            min: { x: -1, y: 0, z: 1 },
            max: { x: 12, y: 12, z: 12 },
    ...
    it("translates, expands, clamps, and converts structurally", () => {
        ...
        expect(
            box.translate({ x: 1, y: 2, z: 3 }).toBlockBoundingBox(),
        ).toEqual({
            min: { x: 1, y: 2, z: 3 },
            max: { x: 2, y: 3, z: 4 },
        });
        expect(box.expandBy(1).toBlockBoundingBox()).toEqual({
            min: { x: -1, y: -1, z: -1 },
            max: { x: 2, y: 2, z: 2 },
        });
        expect(
            box
                .expandToIncludePoint({ x: -2, y: 0.5, z: 3 })
                .toBlockBoundingBox(),
        ).toEqual({
            min: { x: -2, y: 0, z: 0 },
            max: { x: 1, y: 1, z: 3 },
