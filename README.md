...
- `@blurengine/bebe`: engine lifecycle, ownership, runtime scheduling primitives, Audio, Locations, Zones, RenderAnchors, Link, and Metrics
...
- `@blurengine/bebe/maths`: vectors, facings, AABBs, extents, deterministic 3D paths, voxel/grid helpers, tweens, and numeric helpers
...
- Provide the root-level `Audio` singleton for compact BAUD audio packs that tooling compiles and runtime code plays through an explicit `Context`.
...
- Provide the root-level `Locations` registry for immutable generated points and orientations without assigning project meanings to opaque source lines.
...
- [Locations Guide](./docs/guides/locations.md)
